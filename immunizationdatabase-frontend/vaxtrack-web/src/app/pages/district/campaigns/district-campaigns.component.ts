...
export class DistrictCampaignsComponent implements OnInit {
  ...
  createdCampaign: Campaign | null = null;
  ...
  private loadCampaignsInBackground(): void {
    // Reload campaigns without showing loader
    const user = this.authService.getCurrentUser();
    let districtId = user?.districtId;
    const facilityId = user?.facilityId;
    ...
    if (!districtId && facilityId) {
      this.facilityService.getFacilityById(facilityId).subscribe({
        next: (facility) => {
          ...
          if (facilityDistrictId) {
            districtId = facilityDistrictId;
            this.loadCampaignsForDistrictInBackground(facilityDistrictId);
          } else {
            this.loadCampaignsForDistrictInBackground(null);
          }
        },
        error: () => {
          this.loadCampaignsForDistrictInBackground(null);
      ...
      this.loadCampaignsForDistrictInBackground(districtId || null);
    }
  }

  private loadCampaignsForDistrictInBackground(districtId: string | null): void {
    this.campaignService.getAllCampaigns().subscribe({
      ...
      error: (error) => {
        console.error('Failed to reload campaigns:', error);
        // Don't show error message in background reload
      }
    });
  }

  private loadCampaignsForDistrict(districtId: string | null, startTime: number): void {// Clear any previous error messages
    this.errorMessage = '';
    this.campaignService.getAllCampaigns().subscribe({
      next: (campaigns) => {// Clear error message on successful load
        this.errorMessage = '';
        // Filter by district if available
        let filteredCampaigns = campaigns;
        if (districtId) {
          filteredCampaigns = campaigns.filter(c => c.districtId === districtId || c.nationalId);}
        this.campaigns = filteredCampaigns;this.applyFilter();ensureMinimumLoadingTime(startTime, () => {
          this.loading = false;
  ...
  applyFilter(): void {let filtered = [...this.campaigns];
  ...
  onCreateSubmit(): void {
    ...
    const campaign: CreateCampaignRequest = {
      ...
      next: (createdCampaign) => {this.loading = false;
        this.closeCreateModal();
        this.createdCampaign = createdCampaign;
        this.showSuccessModal = true;
        // Reload campaigns in the background without showing loader
