...
export class DistrictCoverageReportComponent implements OnInit {
  ...
  loadFacilities(): void {
    // District dashboard plays admin role - load ALL facilitiesconst user = this.authService.getCurrentUser();
    ...
    this.facilityService.getAllFacilities(true).subscribe({
      ...
      error: (error) => {console.error('Error loading all facilities:', error);
        this.facilities = [];
      }
    });
  }
